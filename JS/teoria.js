...
if (hasCookie("scrollT") && !hasCookie("none")) {
	window.scrollTo(0, getCookie("scrollT"));
	setCookieData('scrollT',{expires: -1 }, ".", 1000*60*60*24);
} 
...
window.addEventListener("unload", function() {
	if (!hasCookie("none")) {
		...
		setCookieData("scrollT", {expires: 1}, window.pageYOffset, 1000*60*60*24);
		setCookieData('check',{expires: 1 }, ".", 3000);
		setCookieData('current',{expires: 1 }, "teoria", 1000*60*60*24);
	} else {
